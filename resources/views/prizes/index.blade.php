...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        const labels = [{!! implode(',', $prizes->pluck('title')->map(function($label) { return "'$label'"; })->toArray() ) !!}];

        const options = {
            responsive: true
        };

        let probabilityChart = document.getElementById("probabilityChart").getContext('2d');
        ...
        new Chart(probabilityChart, {
            ...
            data: {
                labels: labels,
                datasets: [{
                    data: [{{ implode(',', $prizes->pluck('probability')->toArray() ) }}]
                }]
            },
            options: options
        });

        @if( $prizes->sum('alloted') > 0)

            let awardedChart = document.getElementById("awardedChart").getContext('2d');
            ...
            new Chart(awardedChart, {
                ...
                data: {
                    labels: labels,
                    datasets: [{
                        data: [{{ implode(',', $prizes->pluck('alloted')->map(function($value) use ($prizes){ return ( 100 * $value ) / $prizes->sum('alloted'); })->toArray() ) }}]
                    }]
                },
                options: options
            });
